...
class GeneratePluginCommand extends DevToolsCommand{
	...
	public function __construct(){
		parent::__construct(
			"genplugin",
			"Generates skeleton files for a plugin",
	...
	public function execute(CommandSender $sender, string $commandLabel, array $args){
		...
		$rootDirectory = Server::getInstance()->getPluginPath() . $pluginName . DIRECTORY_SEPARATOR;
		if(Server::getInstance()->getPluginManager()->getPlugin($pluginName) !== null or file_exists($rootDirectory)){
		...
		$mainPhpTemplate = self::openSkeleton();
		...
		try{
			...
			$manifest = [
				...
				"version" => "0.0.1",
				"main" => $namespace . "\\Main",
				"api" => Server::getInstance()->getApiVersion(),
	...
	private static function openSkeleton() : mixed{
		//BetterRagnarok ships the skeleton under resources/devtools/plugin_skeleton/Main.php
		$path = \pocketmine\RESOURCE_PATH . "devtools/plugin_skeleton/Main.php";
		if(!file_exists($path)){
			return null;
		}
